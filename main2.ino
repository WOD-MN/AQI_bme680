...
// Altitude at your location (in meters) - MUST ADJUST THIS!
const float ALTITUDE = 200.0; // Replace with your actual altitude
...
void loop(void) {
  if (iaqSensor.run()) {
    ...
    // Create and send JSON payload (now with QNH pressure)
...
// Calculate sea-level pressure (QNH) from station pressure (QFE)
...
void sendSensorData(float temp, float hum, float iaq) {
  ...
  // Calculate QNH (sea-level pressure)
  float stationPressure = iaqSensor.pressure / 100.0; // Convert Pa to hPa (QFE)
  float seaLevelPressure = calculateSeaLevelPressure(stationPressure, ALTITUDE);

  // Debug output (optional)
  Serial.print("Station Pressure (QFE): "); Serial.println(stationPressure);
  Serial.print("Sea-Level Pressure (QNH): "); Serial.println(seaLevelPressure);

  // Create JSON payload
  StaticJsonDocument<256> doc;
  doc["temperature"] = round(temp * 10) / 10.0;
  doc["humidity"] = round(hum * 10) / 10.0;
  doc["pressure"] = round(seaLevelPressure); // Now reports QNH!
